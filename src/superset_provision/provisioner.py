...
def _build_query_context(viz_type: str, dataset_id: int, params: dict) -> str:
    """
    Constrói o query_context necessário para o Superset 4.x executar a query do chart.
    Sem este campo, o chart retorna "Empty query?" ao renderizar.
    """
    # Métricas
    if "metric" in params:       # big_number_total, pie
        metrics = [params["metric"]]
    elif "metrics" in params:    # echarts_bar, echarts_line
    ...
    x_axis  = params.get("x_axis")   # echarts_bar sem time series
    entity  = params.get("entity")   # country_map

    def _col_str(c: Any) -> str:
        """Normaliza coluna para string. Superset 4.x aceita strings diretamente."""
        if isinstance(c, dict):
            return c.get("column_name", str(c))
        return c
    ...
    # Coluna temporal (para aplicar time_range)
    granularity = (
...
def _build_position_json(charts_with_ids: list[dict], title: str = "") -> dict:
    """
    Constrói o position_json do Superset agrupando charts por linha (row)
    e, opcionalmente, por abas (tab).

    Cada item em charts_with_ids deve ter:
        chart_id, slice_name, row, col, width, height, uuid (opcional), tab (opcional)
    """
    ...
    blocks: list[tuple[int, str]] = [(r, f"plain:{r}") for r in plain_rows.keys()]
    ...
    for _, block in sorted(blocks, key=lambda x: (x[0], x[1])):
        if block.startswith("plain:"):
            ...
            _add_row_with_charts(row_id, plain_rows[row_idx], "GRID_ID", ["ROOT_ID", "GRID_ID"])
            continue

        tabs_container_id = "TABS-0"
        positions["GRID_ID"]["children"].append(tabs_container_id)
        positions[tabs_container_id] = {
            "children": [],
        ...
        for tab_idx, tab_name in enumerate(sorted(tabs_rows.keys())):
            tab_id = f"TAB-{tab_idx}"
            positions[tabs_container_id]["children"].append(tab_id)
            positions[tab_id] = {
                "children": [],
                "id": tab_id,
                "meta": {"text": tab_name},
                "parents": ["ROOT_ID", "GRID_ID", tabs_container_id],
                "type": "TAB",
            }

            for row_idx in sorted(tabs_rows[tab_name].keys()):
                row_id = f"ROW-TAB-{tab_idx}-{row_idx}"
                _add_row_with_charts(
                    row_id,
                    tabs_rows[tab_name][row_idx],
                    tab_id,
                    ["ROOT_ID", "GRID_ID", tabs_container_id, tab_id],
                )
...
def _build_native_filters(
    ...
    """
    Converte definições de filtros nativos para o formato JSON do Superset.

    Cada filtro deve ter: name, column, dataset.
    Opcionais: multi_select (default True), description, default_value.
    """
    import datetime

    result = []
    resolved_charts_in_scope = charts_in_scope or []
    for f in filters:
        dataset_id = dataset_name_to_id.get(f["dataset"])
        if not dataset_id:
            log.warning("  Dataset '%s' não encontrado para filtro '%s' — pulando.", f["dataset"], f["name"])
            continue
        ...
        if filter_type == "date":
            ...
            result.append({
                "id": f"NATIVE_FILTER-{uuid.uuid4().hex[:8].upper()}",
                "name": f["name"],
                ...
                "chartsInScope":   resolved_charts_in_scope,
                "tabsInScope":     [],
            })
            continue
        ...
        if raw_default == "current_year":
            default_val = [datetime.datetime.now().year]
